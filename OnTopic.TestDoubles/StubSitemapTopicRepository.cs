...
///   <see cref="ITopicRepository"/>'s own <see cref="ITopicRepository.Load(Int32, Topic?, Boolean, TopicPayload)"/>, requesting
///   the full descendant tree explicitly since <c>isRecursive</c> defaults to <c>false</c>.
/// </remarks>
...
public class StubSitemapTopicRepository(ITopicRepository topicRepository) : ISitemapTopicRepository {
  ...
  public async Task<Topic> Load() {
    var topic                   = await _topicRepository.Load(-1, isRecursive: true).ConfigureAwait(false);

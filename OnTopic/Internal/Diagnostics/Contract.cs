...
namespace OnTopic.Internal.Diagnostics {
  ...
  ///     throughout the OnTopic library. <see cref="Contract"/> does not seek to functionally reproduce Code Contract's <c>
  ///     Ensures()</c>, <c>Assume()</c>, or <c>Invariant()</c>—those methods are not implemented.
  ///   </para>
  ///   <para>
  ...
  ///     that point, nullable reference types should be used in preference for many of the <c>Contract.Requires()</c> calls—
  ///     though, acknowledging, that there are some conditions that won't be satisfied by that (e.g., range checks).
  ///   </para>
  ///   <para>
